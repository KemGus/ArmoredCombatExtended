...
do
	...
	function ACFRadiatorGUIUpdate( _ )
		...
		if not acfmenupanel.CustomDisplay then return end
			...
			local TotalSurfaceArea = finSize * FinCount

			local AirflowRestrictiveness = 1-(1-1/Length)^3 --Airflow ratio of the radiator. Difficulty air flowing through it will have cooling anything.
			...
			acfmenupanel:CPanelText("Area", "Total Fin Surface Area: " .. math.Round(TotalSurfaceArea/12/12,1) .. " ft^2")
			local KJTo100C = Capacity * 4.186 --Heat capacity of the radiator. Joules needed to raise the radiator to 100C.
